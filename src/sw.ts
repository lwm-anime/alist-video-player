...
function getUrlExpiredTime(url: string) {
  ...
  if (amzExpires) {
    ...
    if (amzDate) {
      // example: 20241109T084815Z
      const year = parseInt(amzDate.substring(0, 4), 10);
      const month = parseInt(amzDate.substring(4, 6), 10) - 1; // 月份从0开始
      ...
      const expires = Date.UTC(year, month, day, hours, minutes, seconds) / 1000 + parseInt(amzExpires);
      return expires;
    }
  }
